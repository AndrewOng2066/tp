    ...
    public PersonCard(Person person, int displayedIndex) {
        ...
        if (attendances.size() == 0) {
            ...
            for (Attendance a : attendances) {
                if (a.attendanceName.getStatus().equals("1") || a.attendanceName.getStatus().equals("0")) {
                    presentAndValid++;
                }
            }
            attendancePercentage.setText(String.format("Attendance percentage (Present & Valid): %.2f"
                    + (presentAndValid * 1.0) / attendances.size()));
        }
